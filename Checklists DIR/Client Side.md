...
- Conditions
	...
	- [ ] session ID must be a cookie with the ==SameSite== cookie policy set to "None" or "Lax" -> [[Notes/OLD Notes/WEB/vulnerabilities/CSRF/concepts and defense#Strict SameSite cookies|Concept]]
	- [ ] HTTP request shouldn't carry any ==unpredictable values==

- need POC? ->  [[Notes/OLD Notes/WEB/vulnerabilities/CSRF/attack/Examples#POCs|POC examples]]
...
- [ ]  it checks **`Referer`**? [[Notes/OLD Notes/WEB/vulnerabilities/CSRF/attack/Examples#Referrer-based CSRF|Examples]]
...
- [ ] Change **Content-Type** (<mark style="background: #FFB86CA6;">Json based</mark> CSRF) [[Notes/OLD Notes/WEB/vulnerabilities/CSRF/attack/Examples#Content-Type based CSRF|Examples]]
	- [ ] **`text/plain`**
...
- [ ] is the **csrftoken tied to session**?
	...
	- [ ] is there any **csrfKey cookie**? 
		- [ ] the cookie is not tied to the session ? 
			...
			- [ ] find sink where you can inject cookie [[Notes/OLD Notes/WEB/vulnerabilities/CSRF/attack/Examples#token tied to non-session cookie|Example Senario]] 
