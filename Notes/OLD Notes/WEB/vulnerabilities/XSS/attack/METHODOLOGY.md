...
-> [[Notes/OLD Notes/WEB/vulnerabilities/XSS/attack/Examples#basic attack|basic payload]]
##### PoC
...
- ***reflected examples***
  - **most tags and attributes blocked** -> find accepted tags -> find accepted attributes (burp intruder): / [[Notes/OLD Notes/WEB/vulnerabilities/XSS/attack/Examples#most tags and attributes blocked|payload]]
  - **all tags blocked except custom ones** -> onfocus + attribute that triggers it (like tabindex): / [[Notes/OLD Notes/WEB/vulnerabilities/XSS/attack/Examples#all tags and attributes blocked except custom ones|payload]]
  - **some SVG markup allowed** -> find accepted markup using intruder / [[Notes/OLD Notes/WEB/vulnerabilities/XSS/attack/Examples#some SVG markup allowed|payload]]
...
[[Notes/OLD Notes/WEB/vulnerabilities/XSS/attack/Examples#in HTML tag attributes|payloads]]
## XSS into JavaScript

- <mark style="background: #BBFABBA6;">context: into a JavaScript string</mark>
	- **Terminating the existing script** -> close script tag -> introduce new HTML tag / [[Notes/OLD Notes/WEB/vulnerabilities/XSS/attack/Examples#Terminating the existing script|payload]]
	- **Breaking out of a JavaScript string** -> repair -> a code without error / [[Notes/OLD Notes/WEB/vulnerabilities/XSS/attack/Examples#Breaking out of a JavaScript string|payload]]
	  - single quote escaped with backslash -> escape backslash with another backslash 
- <mark style="background: #BBFABBA6;">context:within a quoted tag attribute & into a JavaScript string</mark> (like event handlers) -> **Making use of HTML-encoding** [[Notes/OLD Notes/WEB/vulnerabilities/XSS/attack/Examples#Making use of HTML-encoding|payload]]
- <mark style="background: #BBFABBA6;">context:in JavaScript template literals</mark> -> use `${...}` / [[Notes/OLD Notes/WEB/vulnerabilities/XSS/attack/Examples#XSS in JavaScript template literals|payload]]
...
[[Notes/OLD Notes/WEB/vulnerabilities/XSS/attack/Examples#to steal cookies|payload]]
## to capture passwords

- only works when user uses password auto-fill (password managers)
[[Notes/OLD Notes/WEB/vulnerabilities/XSS/attack/Examples#to capture passwords|payload]]
...
[[Notes/OLD Notes/WEB/vulnerabilities/XSS/attack/Examples#to perform CSRF|payload]]
