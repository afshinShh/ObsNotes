...
- ***SSRF attacks against the server*** -> HTTP request back to the server that is hosting the application. 
  - **How?** -> URL with <mark style="background: #FFB86CA6;">hostname = loopback</mark>. [[Notes/OLD Notes/WEB/vulnerabilities/SSRF/Examples#against the server|example]]
    - `127.0.0.1`
    - `localhost`
...
- ***SSRF attacks against other back-end systems*** -> interact with back-end systems that are not directly reachable by users.
	- **How?** -> <mark style="background: #FFB86CA6;">non-routable private IP addresses</mark>. [[Notes/OLD Notes/WEB/vulnerabilities/SSRF/Examples#against other back-end systems|example]] 
...
- <mark style="background: #ADCCFFA6;">try to redirect</mark> :
 ...
 [[Notes/OLD Notes/WEB/vulnerabilities/SSRF/Examples#blacklist-based input filters|example]]
...
- application *trusts* the redirection from itself -> request to the desired back-end target [[Notes/OLD Notes/WEB/vulnerabilities/SSRF/Examples#via open redirection|example]]
