...
- Use command seprators + command to retrive data. [[Notes/OLD Notes/WEB/vulnerabilities/Command Injection/payload#basic|example]]
...
- _ping_ command 
	- ex:  `& ping -c 10 127.0.0.1 &` [[Notes/OLD Notes/WEB/vulnerabilities/Command Injection/payload#using time delays|payload]]
## by redirecting output

- `>` , `>>`
	- ex: `& whoami > /var/www/static/whoami.txt &` [[Notes/OLD Notes/WEB/vulnerabilities/Command Injection/payload#by redirecting output|payload]]

## by using out-of-band (OAST)

- *`nslookup`* + inline execution+ a *domain* which the attacker controls: [[Notes/OLD Notes/WEB/vulnerabilities/Command Injection/payload#by using out-of-band (OAST)|payload]]
