...
no defense -> create a form ( [[OLD/WEB/vulnerabilities/CSRF/attack/payload#basic| basic payload]] )
...
- same request from 2 different user =>
  ...
  - change csrfKey cookie -> not related to session ->
    ...
    - find sink where you can inject cookie -> use a html element to deliver crafted link 
      [[OLD/WEB/vulnerabilities/CSRF/attack/payload#token tied to non-session cookie| token tied to non-session cookie]]
...
- [[OLD/WEB/vulnerabilities/CSRF/attack/payload#What's the difference between a site and an origin?|examples]]
